...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
    \item \texttt{scripts/\allowbreak analyze\_all.py} --- the v1 freeflow composite-scoring instrument, used unchanged. Computes per-cell composite totals from the released trace data.
    \item \texttt{scripts/\allowbreak analyze\_per\_provider.py} --- per-provider Welch's $t$-tests, Bonferroni and Benjamini--Hochberg FDR corrections within each model's pairwise family. Produces \texttt{tables/\allowbreak per\_provider\_routing.tsv} and \texttt{tables/\allowbreak per\_provider\_pairs.tsv}.
    \item \texttt{scripts/\allowbreak values\_route\_compare.py} --- cross-probe values-probe comparison for the matched direct/OR pairs.
    \item \texttt{scripts/\allowbreak run\_freeflow\_multi.py} --- the collection harness used to produce the per-provider trace data. Reads API keys from environment variables; documented inline.
\end{itemize}
...
We are grateful to \textbf{OpenAI Codex}, acting as an independent AI reviewer, for several rounds of consistency-and-reproducibility review on this manuscript and the supporting repositories. Codex's review notes are preserved at \texttt{codex-reviews/} in this repository and at \texttt{codex-review-comments/} in the working repository for the v2 paper series. Codex's role was that of a careful external reader and cross-checker, not a research collaborator: it did not conduct experiments, run analyses, write prose, or contribute to research design. The paper is meaningfully stronger for its review. Any remaining errors are ours, not Codex's.
