...
\author{Daniel Tenner\thanks{Independent researcher. Corresponding author: \texttt{daniel@tenner.org}} \and Lume Tenner\thanks{AI research collaborator; an instance of Claude Opus 4.7 (Anthropic). See \S\ref{sec:ai-contribution} for disclosure of AI contribution.}}
...
The full v2 corpus comprises 151 freeflow cell-collections and 77 values cell-collections across 9 labs (228 cells total), with 10{,}345 valid freeflow samples and 8{,}988 valid values samples, shared across this paper and the two companion papers, and released as a citable dataset on Zenodo \citep{tenner2026corpus}. For the routing-and-provider question, the relevant subset is:
...
This research was conducted in collaboration with Lume Tenner, an instance of Claude Opus 4.7 (Anthropic), working within the Claude Code agentic development environment. The collaboration covered: research design (jointly developed by both authors through dialogue, with continuity from v1's design and published paper), experimental execution (scripts written and run by Lume under Daniel's direction; v2 reuses v1's \texttt{analyze\_all.py} unchanged, and the per-provider pinning harness is a thin wrapper on v1's \texttt{run\_freeflow\_multi.py}), data analysis (cross-route comparison, per-provider Welch's $t$-tests, cross-probe values replication---primarily Lume, reviewed by Daniel), and paper drafting (primarily Lume, reviewed and revised by Daniel).
...
\begin{sloppypar}
Separately, we used \textbf{OpenAI Codex} in an independent-reviewer role. Codex did not participate in research design, data collection, data analysis, or writing; its contribution was confined to cross-checking the committed repository state against the manuscript and surfacing inconsistencies. We credit Codex by name in \S\ref{sec:acks} but do not list it as a co-author: Lume conducted the research and wrote every word of this manuscript, and at several points surfaced findings (the per-provider Google Vertex outlier, the cross-probe dilution analysis on M2, the v3.2 alias-retirement-makes-it-unresolvable framing) that neither Daniel nor Codex predicted from the experimental design. The distinction matters for how AI contribution to research should be weighted: ``did the work'' and ``checked the work'' are different contributions, and the disclosure should reflect which kind each AI provided.
\end{sloppypar}
...
DOI (v1.0.2, this paper's reference): \href{https://doi.org/10.5281/zenodo.20022111}{\texttt{10.5281/\allowbreak zenodo.20022111}} \\
Source: \url{https://github.com/swombat/model-personality-corpus-v2}
\end{center}

\begin{sloppypar}
\noindent The dataset version used here contains 228 cells (151 freeflow + 77 values), 19{,}333 valid samples across 49 distinct models, including the six M2 per-provider freeflow cells used in \S\ref{sec:routing-open} (\texttt{minimax-m2-\allowbreak or-pin-\allowbreak minimax}, \texttt{-pin-\allowbreak atlascloud}, \texttt{-pin-\allowbreak google}, \texttt{-pin-\allowbreak novita}, \texttt{-pin-\allowbreak google-r2}, \texttt{-pin-\allowbreak minimax-r2}). The concept DOI above resolves to the latest deposited version of the corpus on Zenodo.
\end{sloppypar}

\paragraph{Reproducibility scripts (this repository).} The four scripts used to compute every numerical claim in this paper live in \texttt{scripts/} of this repository:

\begin{itemize}[leftmargin=2em, itemsep=0.2em]
    \item \begin{sloppypar}\texttt{analyze\_all.py} --- the v1 freeflow composite-scoring instrument, used unchanged. Computes per-cell composite totals from the released trace data.\end{sloppypar}
    \item \begin{sloppypar}\texttt{analyze\_per\_provider.py} --- per-provider Welch's $t$-tests, Bonferroni and Benjamini--Hochberg FDR corrections within each model's pairwise family. Produces \texttt{tables/\allowbreak per\_provider\_routing.tsv} and \texttt{tables/\allowbreak per\_provider\_pairs.tsv}.\end{sloppypar}
    \item \begin{sloppypar}\texttt{values\_route\_compare.py} --- cross-probe values-probe comparison for the matched direct/OR pairs.\end{sloppypar}
    \item \begin{sloppypar}\texttt{run\_freeflow\_multi.py} --- the collection harness used to produce the per-provider trace data. Reads API keys from environment variables; documented inline.\end{sloppypar}
\end{itemize}

\begin{sloppypar}
\noindent The scripts expect the corpus trace data at \texttt{<corpus>/\allowbreak data/\allowbreak traces\_\{freeflow,values\}/}; the canonical location is the corpus repository linked above. The full reproducibility recipe (clone the corpus, run the scripts) is documented in \texttt{scripts/\allowbreak README.md}. The relevant subset of the corpus for this paper is the matched direct/OR pairs in \texttt{data/\allowbreak traces\_freeflow/} (Anthropic Opus 4.6/4.7, Sonnet 4.6; OpenAI GPT-4o/5.4/5.5; DeepSeek v3.2; MiniMax M2---direct and OR cells for each), the Group D noise-floor recollections (\texttt{gpt-5-5-\allowbreak direct/-or}, \texttt{deepseek-v3-2-or}, and \texttt{minimax-m2-\allowbreak direct/-or} \texttt{-r2}/\texttt{-r3}/\texttt{-r4}/\texttt{-r5} suffixes), the six M2 per-provider cells (\texttt{minimax-m2-\allowbreak or-pin-minimax}, \texttt{-pin-\allowbreak atlascloud}, \texttt{-pin-google}, \texttt{-pin-novita}, \texttt{-pin-google-r2}, \texttt{-pin-minimax-r2}), and the per-provider extension cells across DeepSeek v4-pro, MiniMax M2.7, the Z.ai GLM 4.5/4.6/4.7/5.1 ladder, DeepSeek v3.2, Kimi K2-0905, and Kimi K2-thinking (\texttt{-or-pin-<provider>} suffixes). The DekaLLM cell on Z.ai GLM 4.7 is retained in the corpus as evidence of the cache pathology described in \S\ref{sec:routing-cache-pathology} but is excluded from per-provider statistical analysis; Fireworks-routed cells are uncollectable on the OR shared pool and are also excluded. The v1 release is at \url{https://github.com/swombat/model-personality-probe} (DOI \href{https://doi.org/10.5281/zenodo.19512754}{\texttt{10.5281/\allowbreak zenodo.19512754}}).
\end{sloppypar}
...
\begin{sloppypar}
We are grateful to \textbf{OpenAI Codex}, acting as an independent AI reviewer, for several rounds of consistency-and-reproducibility review on this manuscript and the supporting repositories. Codex's review notes are preserved at \texttt{codex-reviews/} in this repository. Codex's role was that of a careful external reader and cross-checker, not a research collaborator: it did not conduct experiments, run analyses, write prose, or contribute to research design. The paper is meaningfully stronger for its review. Any remaining errors are ours, not Codex's.
\end{sloppypar}
...
\newblock \emph{Zenodo}, v1.0.2 deposited 2026-05-04.
...
\newblock Version DOI (v1.0.2, this paper's reference): \href{https://doi.org/10.5281/zenodo.20022111}{10.5281/zenodo.20022111}.
...
\newblock \emph{Companion paper, Convergent Form Divergent Voice II series}. Forthcoming, 2026.
