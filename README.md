...
- **Per-provider routing analysis** (paper §4.3, §4.4, §4.6;
  Tables 4, 5 and the within-OR pairwise *d* / *t* / *p* values) —
  corpus's `tables/per_provider_routing.md` and
  `tables/per_provider_pairs.tsv`, generated by
  `scripts/analyze_per_provider.py`. Both probes (`--probe both`)
  produce the values-probe replication used in §4.4 and §4.7.
...
- **Cross-probe values replication** (paper §4.5 dilution analysis,
  Table 7) — produced by the corpus's
...
review notes on the unified pre-split paper (preserved in the working
repository at
[`codex-review-comments/`](https://github.com/swombat/model-personality-probe-v2/tree/master/codex-review-comments)),
and for the round of review on this split's published artefact. Codex's
contribution was confined to cross-checking the committed repository
state against the paper; it did not participate in research design,
data collection, analysis, or writing. The distinction between "did the
work" (Lume) and "checked the work" (Codex) is reflected in the byline:
Codex is acknowledged here, not listed as a co-author. Any remaining
